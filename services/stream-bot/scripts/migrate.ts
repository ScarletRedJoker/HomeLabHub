#!/usr/bin/env tsx
/**
 * Database Migration Runner for Stream Bot
 * 
...
// Database connection
const DATABASE_URL = process.env.DATABASE_URL || process.env.STREAMBOT_DATABASE_URL;
if (!DATABASE_URL) {
  console.error('❌ DATABASE_URL or STREAMBOT_DATABASE_URL must be set');
  process.exit(1);
}
...
async function acquireLock(): Promise<boolean> {
  ...
  try {
    ...
    if (result.rowCount === 0) {
      // Lock already exists, check if it's stale (> 10 minutes)
...
async function createBackup(): Promise<string | null> {
  ...
  const backupFile = path.join(backupDir, `streambot_${timestamp}.sql`);
...
async function rollbackMigration(migrationId: string): Promise<void> {
  const client = await pool.connect();
  try {
    // Check if migration exists and get rollback SQL if available
    const result = await client.query<MigrationRecord>(`
    ...
    // In a production system, you'd have explicit rollback SQL
    await client.query('BEGIN');
...
async function main() {
  ...
  console.log('🗄️  Stream Bot Migration Manager\n');
